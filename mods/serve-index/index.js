...
function createHtmlFileList(files, dir, useIcons, view) {
  ...
  html += files.map(function (file) {
    ...
    return '<li><a href="'
      ...
      + ' id="id_link_' + escapeHtml(file.name) + '"'
...
function createHtmlLinkScriptList(files) {
  var html ='';
  html += files.map(function (file) {
    return 'document.getElementById("id_link_' + escapeHtml(file.name) + '").href += "?x_auth_token=" + localStorage.getItem("antentafm_token");';
  }).join('\n');
  ...
  return function render(locals, callback) {
    ...
    fs.readFile(template, 'utf8', function (err, str) {
      ...
      var body = str
        ...
        .replace(/\{script\}/g, createHtmlLinkScriptList(locals.fileList))
